from dataset import fashionMNIST_test, fashionMNIST_train
...
# %% Training
# Full Data and AUM
data_train_full = fashionMNIST_train()
Model_full = Model_MNIST().to(device)
...
train_dataloader_full = torch.utils.data.DataLoader(dataset=data_train_full,
                                                 batch_size=batch_size,
                                                 shuffle=True)
data_train_aum = fashionMNIST_train()
Model_aum = Model_MNIST().to(device)
...
data_train_random = fashionMNIST_train(remain=random.choices(data_train_full.remain,k=int(len(train_dataloader_full)/2)))
Model_random = Model_MNIST()
