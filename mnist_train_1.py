...
data_train_random = MNIST_train(remain=random.choices(data_train_full.remain,k=int(len(train_dataloader_full)/2)))
...
for t in range(epochs):
    ...
    data_train_aum = MNIST_train(remain=sp)
    train_dataloader_aum = torch.utils.data.DataLoader(dataset=data_train_aum,
                                                       batch_size=batch_size,
                                                       shuffle=True)
